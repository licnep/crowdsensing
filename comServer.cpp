...
//TODO: Decidere dove mettere le credenziali di accesso. Eliminare i metodi checkAuthorization e Authorization???
//tanto tutto viene fatto nella sendMessage nei metodi put e post. 
//BUGS da segnalare:
...
public:
    ...
    {        
        ...
        if (method==POST)
        {   string username="gruppo35";  
	    string password="034FpK69l4";
            curl_easy_setopt(curl, CURLOPT_URL, APIendpoint.c_str());
...
public:
    ...
    void modifyDevice()
    {	
	string json = "{\"username\":\"gruppo35\",\"raspb_wifi_mac\":\""+raspb_wifi_mac+"\"}";
        string result = cw.sendMessage(CurlWrapper::PUT,baseURL + "/devices/FF",json);
...
//                    "},"
    ...
    {	
	cout << "Eseguo l'autenticazione" <<endl;
...
int main(int argc, char** argv) 
{
	...
	//cs.addDevice();    
	cs.modifyDevice();
	cs.getDeviceInfo("b8:27:eb:69:a4:20");
